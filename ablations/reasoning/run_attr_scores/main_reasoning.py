...
class BaselinesReasoning:
    def __init__(self, attr_score_function: str):
    ...
    def run(self, data):
        ...
        for i, row in enumerate(data):
            ...
            begin = time.time()
            all_labels.append(model_pred_origin.cpu())
            ...
            words = [self.tokenizer.convert_ids_to_tokens(input_ids[0, i].cpu().unsqueeze(0)) for i in torch.argsort(eval_attr_score.cpu(), descending=True).tolist()]
            words = [w[0] for w in words]
            all_important_words.append(words)
            end = time.time()
            all_times.append(end - begin)
