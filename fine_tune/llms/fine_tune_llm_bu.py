...
class LlmFineTune:
    ...
    def __init__(self, task: Task, is_bf16: bool, is_use_prompt: bool, model_backbone: str, n_epochs: int):
        ...
        self.train_ds = self.tokenize_ds(dataset["train"].select(range(90)).shuffle(seed = 42))
        self.test_ds = self.tokenize_ds(dataset["test"].select(range(15)))
    ...
    def run(self):
        ...
        model_to_merge = PeftModel.from_pretrained(
            MistralForSequenceClassification.from_pretrained(self.model_path, num_labels = self.num_labels,
                                                             torch_dtype = torch.bfloat16, cache_dir = HF_CACHE),
        ...
        # Save the model
        model_to_merge.save_pretrained(f"{self.output_dir}/merged_model")
