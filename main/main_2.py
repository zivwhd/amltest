...
class Baselines:
    def __init__(self, exp_name: str, attr_score_function: str, metrics: List[EvalMetric], data = None):
        ...
        for i, row in enumerate(self.data):
            ...
            if ExpArgs.attribution_scores_function == AttrScoreFunctions.sequential_integrated_gradients.value:
                ...
                if is_model_encoder_only():
                    print("C" * 100)
                    _attr = explainer.attribute(input_embed, internal_batch_size = 3, baselines = ref_input_embed,
                                                n_steps = n_steps, additional_forward_args = (
                    ...
                    if batch_size != 0:
                        print("K" * 100)
                        _attr = explainer.attribute(input_embed, internal_batch_size = batch_size,
            ...
            if ExpArgs.is_evaluate:
                ...
                for metric in self.metrics:
                    ...
                    if ExpArgs.is_save_results:
                        ...
                        evaluation_item["attribution_scores"] = str([f"{i:.10f}" for i in attribution_scores.tolist()])
                        evaluation_item["tokens"] = str(
                            [self.tokenizer.convert_ids_to_tokens(item) for item in input_ids])
                        print(f"attribution_scores: {attribution_scores}")
                        k = min(10, attribution_scores.squeeze().shape[-1])
                        topk = input_ids.squeeze()[torch.topk(attribution_scores, k).indices.tolist()]
                        evaluation_item["most_important_words"] = str(
                            [self.tokenizer.convert_ids_to_tokens(item.item()) for item in topk])
    ...
    def solvability_func(self, sentences):
        ...
        tok = self.tokenizer(sentences, truncation=True, return_tensors = 'pt', padding = True, add_special_tokens = False).to(
            self.device)
        with torch.no_grad():
